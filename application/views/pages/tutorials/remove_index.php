...
<p>This example is quite dynamic in that you can add files and directories to your document root as you desire and you'll never need to modify the rewrite rules.  Any files and directories that exist under your document root will be served.  If a request is made for a non-existant file or directory (which is really what the index.php-less Kohana URLs are), the request is rewritten to be routed through index.php transparently.  If it can be routed by Kohana, the page is served.  Finally, if it wasn't a request for an existing file or directory and could not be routed by Kohana, a Kohana error page (ex: 404) is displayed.  So you not only have dynamic rewrite rules, but you get consistency in your error pages site wide.</p>
<p><em>However</em>, this approach does not protect your more sensitive an unintended PHP files against access.  So someone could enter, for example, <tt>http://www.example.com/application/views/</tt> and get a list of all your view files.  Ideally, you should never have your system or application directories, or any other files you do not want accessed, under your document root.  But some servers are setup such that your access to the server is restricted to document root and you have no choice.  Read <?php echo html::anchor('http://docs.kohanaphp.com/installation','&quot;Moving system and application directory out of webroot&quot; at http://docs.kohanaphp.com/installation'); ?> to learn how to move <tt>system/</tt> and <tt>application/</tt> out of your document root.  If you find that you do not have proper access on your server to change your file system setup, continue on in this tutorial for other options.</p>
...
<p>Now we are really done. You can change the allowed files and directories to whatever your own, specific needs are.</p>
