...
class Core_Loader {
	...
	var $_ob_level;
	var $_view_path		= '';
	var $_is_instance = FALSE; // Whether we should use $this or $CORE = Kohana::$instance
	...
	public function __construct()
	{
		...
		Log::add('debug', 'Loader Class Initialized');
	}
	...
	public function helper($helpers = array())
	{
		...
		Log::add('debug', 'Helpers loaded: '.implode(', ', $helpers));
	}
	...
	public function plugin($plugins = array())
	{
		...
		Log::add('debug', 'Plugins loaded: '.implode(', ', $plugins));
	}
	...
	public function script($scripts = array())
	{
		...
		Log::add('debug', 'Scripts loaded: '.implode(', ', $scripts));
	}
	 ...
	 * @return	void
	 */
	public function language($file = '', $lang = '', $return = FALSE)
	{
		$CORE = Kohana::$instance;
	...
	private function _load($data)
	{
		...
		Log::add('debug', 'File loaded: '.$path);
	 ...
	 * @return 	void
	 */
	private function _load_class($class, $params = NULL)
	{
		// Get the class name
		...
		foreach (array($class, ucfirst($class)) as $class)
		{
			...
			{
				Log::add('error', "Unable to load the requested class: ".$class);
			...
			if ($fp === TRUE)
			{
				Log::add('debug', $class." class already loaded. Second attempt ignored.");
				return;
			}
	 ...
	 * @return 	void
	 */
	private function _find_class($class)
	{
	 ...
	 * @return 	void
	 */
	private function _find_driver($library, $name)
	{
	 ...
	 * @return	null
	 */
	private function _init_class($class, $config = FALSE)
	{
	 ...
	 * @return	array
	 */
	private function _assign_to_models()
	{
