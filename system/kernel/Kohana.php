...
// Test if Kohana is running in Windows
...
abstract class Kohana {
	...
	public static function setup()
	{
		...
		\Kernel\Benchmark::start(SYSTEM_BENCHMARK.'_environment_setup');
		...
		\Kernel\Kohana_Config::instance();

		if (\Kernel\Kohana_Config::instance()->loaded() === FALSE)
		{
		...
		// Set autoloader
		spl_autoload_register(array('\Kernel\Kohana', 'auto_load'));

		// Register a shutdown function to handle system.shutdown events
		register_shutdown_function(array('\Kernel\Kohana', 'shutdown'));
		...
		\Kernel\Kohana_Exception::enable();

		// Enable error handling
		\Library\Kohana_PHP_Exception::enable();
		...
		\Library\I18n::set_locale(Kohana::$locale);
		...
		Event::add('system.routing', array('\Library\Router', 'find_uri'));
		Event::add('system.routing', array('\Library\Router', 'setup'));

		// Enable Kohana controller initialization
		Event::add('system.execute', array('\Kernel\Kohana', 'instance'));

		// Enable Kohana 404 pages
		Event::add('system.404', array('\Library\Kohana_404_Exception', 'trigger'));
		...
		\Kernel\Benchmark::stop(SYSTEM_BENCHMARK.'_environment_setup');
	}
	...
	public static function cleanup()
	{
		...
		\Kernel\Kohana_Exception::disable();

		\Library\Kohana_PHP_Exception::disable();

		spl_autoload_unregister(array('\Kernel\Kohana', 'auto_load'));
	...
	public static function & instance()
	{
		...
		{
			\Kernel\Benchmark::start(SYSTEM_BENCHMARK.'_controller_setup');

			// Include the Controller file
			require_once \Library\Router::$controller_path;

			try
			{
				// Start validation of the controller
				$class = new \ReflectionClass('\Controller\\'.ucfirst(\Library\Router::$controller));
			}
			...
			\Kernel\Event::run('system.pre_controller');

			// Create a new controller instance
			$controller = $class->newInstance();

			// Controller constructor has been executed
			\Kernel\Event::run('system.post_controller_constructor');

			try
			{
				// Load the controller method
				$method = $class->getMethod(\Library\Router::$method);

				// Method exists
				if (\Library\Router::$method[0] === '_')
				{
				...
				// Default arguments
				$arguments = \Library\Router::$arguments;
			}
			catch (ReflectionException $e)
			{
				// Use __call instead
				...
				$arguments = array(\Library\Router::$method, \Library\Router::$arguments);
			}

			// Stop the controller setup benchmark
			\Kernel\Benchmark::stop(SYSTEM_BENCHMARK.'_controller_setup');

			// Start the controller execution benchmark
			\Kernel\Benchmark::start(SYSTEM_BENCHMARK.'_controller_execution');

			// Execute the controller method
			$method->invokeArgs($controller, $arguments);

			// Controller method has been executed
			\Kernel\Event::run('system.post_controller');

			// Stop the controller execution benchmark
			\Kernel\Benchmark::stop(SYSTEM_BENCHMARK.'_controller_execution');
		}
	...
	{
		return \Kernel\Kohana_Config::instance()->get($key,$slash,$required);
	}
	...
	public static function output_buffer($output)
	{
		...
		if ( ! \Kernel\Event::has_run('system.send_headers'))
		{
			// Run the send_headers event
			\Kernel\Event::run('system.send_headers');
		}
	...
	public static function shutdown()
	{
		...
		\Kernel\Event::run('system.display', Kohana::$output);
	...
	public static function render($output)
	{
		if (Kohana::config('core.render_stats') === TRUE)
		{
			...
			$benchmark = \Kernel\Benchmark::get(SYSTEM_BENCHMARK.'_total_execution');
		...
		{
			if ($compress = \Helper\request::preferred_encoding(array('gzip','deflate'), TRUE))
			{
	...
	public static function auto_load($class)
	{
		...
		switch ($namespace)
		{
			...
			default:
				$type = ($file[0] < 'a') ? 'libraries' : 'helpers';
				break;
		}
