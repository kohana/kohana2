...
class Kohana_Exception extends \Exception {
	...
	public function __construct($message, array $variables = NULL, $code = 0)
	{
		...
		$message = \Library\I18n::__($message, $variables);
	...
	public static function enable()
	{
		...
		{
			set_exception_handler(array('\Kernel\Kohana_Exception', 'handle'));
	...
	public static function handle(\Exception $e)
	{
		try
		{
			...
			\Library\Kohana_Log::add('error', $error);

			// Manually save logs after exceptions
			\Library\Kohana_Log::save();

			if (\Kernel\Kohana::config('kohana/core.display_errors') === FALSE)
			{
			...
			if ($e instanceof Kohana_Exception)
			{
				...
				$code = \Kernel\Kohana::message('kohana/core.errors.'.$code);
			}
			else
			{
				$template = \Kernel\Kohana_Exception::$template.$template;
				...
				{
					// Use the human-readable error name
					$code = \Kernel\Kohana::message('kohana/core.errors.'.$e->getSeverity());
			...
			if ($template = \Kernel\Kohana::find_file('views', $template))
			{
				include $template;
			}
		}
		catch (\Exception $e)
		{
			...
			echo \Kernel\Kohana_Exception::text($e), "\n";
		}
	...
	public static function debug_path($file)
	{
		...
		elseif (strpos($file, PKGPATH) === 0)
		{
			$file = 'PKGPATH/'.substr($file, strlen(PKGPATH));
		}
