...
class feed {
	...
	public static function parse($feed, $limit = 0)
	{
		...
		if( ! function_exists('simplexml_load_file'))
			throw new \Library\Kohana_User_Exception('Feed Error', 'SimpleXML must be installed!');
		...
		$load = (is_file($feed) OR \Helper\valid::url($feed)) ? 'simplexml_load_file' : 'simplexml_load_string';
	...
	public static function create($info, $items, $format = 'rss2', $encoding = 'UTF-8')
	{
		...
		foreach ($info as $name => $value)
		{
			...
			elseif (($name === 'link' OR $name === 'docs') AND strpos($value, '://') === FALSE)
			{
				...
				$value = \Helper\url::site($value, 'http');
			}
