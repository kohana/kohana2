...
class download_Core {
	 ...
	 *
	 * @uses exit()
	 * @uses \Kernel\Kohana::close_buffers()
	 *
	...
	public static function send($filename, $data = NULL)
	{
		...
		$mime = \Kernel\Kohana::config('mimes.'.strtolower(substr(strrchr($filename, '.'), 1)));
		$mime = empty($mime) ? 'application/octet-stream' : $mime[0];

		// Close output buffers
		\Kernel\Kohana::close_buffers(FALSE);

		// Clear any output
		Event::add('system.display', create_function('', '\Kernel\Kohana::$output = "";'));
