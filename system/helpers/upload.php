...
class upload {
	...
	public static function save($file, $filename = NULL, $directory = NULL, $chmod = 0644)
	{
		...
		if (\Kernel\Kohana::config('upload.remove_spaces') === TRUE)
		{
		...
		if ($directory === NULL)
		{
			...
			$directory = \Kernel\Kohana::config('upload.directory', TRUE);
		}

		// Make sure the directory ends with a slash
		$directory = rtrim($directory, '/').'/';

		if ( ! is_dir($directory) AND \Kernel\Kohana::config('upload.create_directories') === TRUE)
		{
		...
		if ( ! is_writable($directory))
			throw new \Kernel\Kohana_Exception('The upload destination folder, :dir:, does not appear to be writable.', array(':dir:' => $directory));
