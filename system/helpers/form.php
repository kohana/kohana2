...
class form_Core {
	...
	public static function textarea($data, $value = '', $extra = '', $double_encode = TRUE)
	{
		...
		return '<textarea'.form::attributes($data, 'textarea').' '.$extra.'>'.htmlspecialchars($value, ENT_QUOTES, \Kernel\Kohana::CHARSET, $double_encode).'</textarea>';
	}
		...
		foreach ((array) $options as $key => $val)
		{
			...
			if (is_array($val))
			{
				...
				foreach ($val as $inner_key => $inner_val)
				{
					...
					$input .= '<option value="'.$inner_key.'"'.$sel.'>'.htmlspecialchars($inner_val, ENT_QUOTES, \Kernel\Kohana::CHARSET, FALSE).'</option>'."\n";
				}
				$input .= '</optgroup>'."\n";
			}
			else
			{
				...
				$input .= '<option value="'.$key.'"'.$sel.'>'.htmlspecialchars($val, ENT_QUOTES, \Kernel\Kohana::CHARSET, FALSE).'</option>'."\n";
			}
		}
		$input .= '</select>';
