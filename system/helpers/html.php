...
class html {
	...
	public static function chars($str, $double_encode = TRUE)
	{
		...
		return htmlspecialchars($str, ENT_QUOTES, \Kernel\Kohana::CHARSET, $double_encode);
	}
	...
	public static function anchor($uri, $title = NULL, $attributes = NULL, $protocol = NULL, $escape_title = FALSE)
	{
		if ($uri === '')
		{
			$site_url = \Helper\url::base(FALSE);
		}
		elseif (strpos($uri, '#') === 0)
		{
		...
		{
			$site_url = \Helper\url::site($uri, $protocol);
		}
		else
		{
		...
		return
		// Parsed URL
		'<a href="'.htmlspecialchars($site_url, ENT_QUOTES, \Kernel\Kohana::CHARSET, FALSE).'"'
		// Attributes empty? Use an empty string
		.(is_array($attributes) ? html::attributes($attributes) : '').'>'
		// Title empty? Use the parsed URL
		.($escape_title ? htmlspecialchars((($title === NULL) ? $site_url : $title), ENT_QUOTES, \Kernel\Kohana::CHARSET, FALSE) : (($title === NULL) ? $site_url : $title)).'</a>';
	}

	/**
	 * Creates links to non-Kohana resources. This function works the same as
	 * [html::anchor], except it uses [\Helper\url::base] instead of [\Helper\url::site]
	 *
	...
	public static function file_anchor($file, $title = NULL, $attributes = NULL, $protocol = NULL)
	{
		...
		'<a href="'.htmlspecialchars(\Helper\url::base(FALSE, $protocol).$file, ENT_QUOTES, \Kernel\Kohana::CHARSET, FALSE).'"'
		// Attributes empty? Use an empty string
		.(is_array($attributes) ? html::attributes($attributes) : '').'>'
	 ...
	 * ##### Example
	 *
	 *     echo \Kernel\Kohana::debug(html::email('test@mydomain.com'));
	 *
	 *     // Output:
	 ...
	 * `\Library\Router::$segments` if no segment argument is provided.
	 *
	 * ##### Example
	 *
	 *     echo \Kernel\Kohana::debug(html::breadcrumb())
	 *
	...
	{
		$segments = empty($segments) ? \Library\Router::$segments : $segments;
		...
		while ($segment = array_pop($segments))
		{
			...
			(
				...
				ucwords(\Helper\inflector::humanize($segment))
			);
		}
	...
	public static function meta($tag, $value = NULL)
	{
		...
		$attr = in_array(strtolower($tag), \Kernel\Kohana::config('http.meta_equiv')) ? 'http-equiv' : 'name';
	 ...
	 * and makes them absolute using [\Helper\url::base]. If you need to route the
	...
	public static function link($href, $rel, $type, $media = FALSE, $index = FALSE)
	{
		...
		else
		{
			if (strpos($href, '://') === FALSE)
			{
				...
				$href = \Helper\url::base($index).$href;
			}
	...
	public static function script($script, $index = FALSE)
	{
		...
		else
		{
			if (strpos($script, '://') === FALSE)
			{
				...
				$script = \Helper\url::base((bool) $index).$script;
			}
	...
	public static function image($src, $alt = NULL, $index = FALSE)
	{
		...
		if (strpos($attributes['src'], '://') === FALSE)
		{
			...
			$attributes['src'] = \Helper\url::base($index).$attributes['src'];
		}
	...
	public static function attributes($attrs)
	{
		...
		{
			$compiled .= ' '.$key.'="'.htmlspecialchars($val, ENT_QUOTES, \Kernel\Kohana::CHARSET).'"';
		}
