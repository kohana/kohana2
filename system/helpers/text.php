...
class text {
	...
	public static function random($type = 'alnum', $length = 8)
	{
		...
		$pool = ($utf8 === TRUE) ? \Helper\utf8::str_split($pool, 1) : str_split($pool, 1);
	...
	public static function auto_link_urls($text)
	{
		...
		while (preg_match($regex, $text, $match, PREG_OFFSET_CAPTURE, $position))
		{
			...
			// Add the hyperlink.
			$result .= \Helper\html::anchor($full_url, $url);
	...
	public static function auto_link_emails($text)
	{
		...
		if (preg_match_all('~\b(?<!href="mailto:|">|58;)(?!\.)[-+_a-z0-9.]++(?<!\.)@(?![-.])[-a-z0-9.]+(?<!\.)\.[a-z]{2,6}\b~i', $text, $matches))
		{
			foreach ($matches[0] as $match)
			{
				...
				$text = str_replace($match, \Helper\html::mailto($match), $text);
			}
		}
