...
class request {
	...
	public static function referrer($default = FALSE, $remove_base = FALSE)
	{
		if ( ! empty($_SERVER['HTTP_REFERER']))
		{
			...
			if ($remove_base === TRUE AND (strpos($ref, \Helper\url::base(FALSE)) === 0))
			{
				// Remove the base URL from the referrer
				$ref = substr($ref, strlen(\Helper\url::base(FALSE)));
			}
		}
	 ...
	 * ###### Example
	 *
	 *     \Kernel\Kohana::debug(request::is_ajax());
	 *
	 *     // Output:
	 *     (boolean) false
	 *
	...
	public static function method()
	{
		...
		if ( ! in_array($method, request::$http_methods))
			throw new \Kernel\Kohana_Exception('Invalid request method :method:', array(':method:' => $method));
	...
	public static function user_agent($key = 'agent')
	{
		...
		if ( ! is_array(request::$user_agent))
		{
			...
			foreach (\Kernel\Kohana::config('user_agents') as $type => $data)
			{
	 ...
	 *     \Kernel\Kohana::debug(request::accepts('application/xhtml+xml'));
	 *
	 *     // Output:
	 *     (boolean) true
	 *
	 *     // With no type specified
	 *     \Kernel\Kohana::debug(request::accepts());
	 *
	 *     // Output:
	 *     (array) Array
	 *     (
	 ...
	 *     \Kernel\Kohana::debug(request::accepts_charset('UTF-8'));
	 *
	 *     // Output:
	 *     (boolean) true
	 *
	 *     // With no function argument
	 *     \Kernel\Kohana::debug(request::accepts_charset());
	 *
	 *     // Output:
	 *     (array) Array
	 *     (
	 ...
	 *     \Kernel\Kohana::debug(request::accepts_encoding('gzip'));
	 *
	 *     // Output:
	 *     (boolean) true
	 *
	 *     // With no function argument
	 *     \Kernel\Kohana::debug(request::accepts_encoding());
	 *
	 *     // Output:
	 *     (array) Array
	 *     (
	 ...
	 *     \Kernel\Kohana::debug(request::accepts_language('en'));
	 *
	 *     // Output:
	 *     (boolean) true
	 *
	 *     // With no function argument
	 *     \Kernel\Kohana::debug(request::accepts_language());
	 *
	 *     // Output:(array) Array
	 *     (
	 *         [en] => Array
	 ...
	 * ###### Example
	 *
	 *     \Kernel\Kohana::debug(request::accepts_at_quality('application/xhtml+xml));
	 *
	 *     // Output:
	 *     (integer) 1
	 *
	 *     \Kernel\Kohana::debug(request::accepts_at_quality('text/html'));
	 *
	 *     // Output:
	 *     (double) 0.9
	 *
	...
	public static function accepts_at_quality($type, $explicit_check = FALSE)
	{
		...
		if (strpos($type, '/') === FALSE)
		{
			...
			foreach ((array) \Kernel\Kohana::config('mimes.'.$type) as $type)
			{
	 ...
	 * ###### Example
	 *
	 *     \Kernel\Kohana::debug(request::accepts_charset_at_quality('utf-8'));
	 *
	 *     // Output:
	 *     (integer) 0.7
	 *
	 *     \Kernel\Kohana::debug(request::accepts_charset_at_quality('iso-8859-1'));
	 *
	 *     // Output:
	 *     (integer) 1
	 *
	 ...
	 * ###### Example
	 *
	 *     \Kernel\Kohana::debug(request::accepts_encoding_at_quality('gzip'));
	 *
	 *     // Output:
	 *     (integer) 1
	 *
	 *     \Kernel\Kohana::debug(request::accepts_encoding_at_quality('deflate'));
	 *
	 *     // Output:
	 *     (integer) 1
	 *
	 ...
	 * ###### Example
	 *
	 *     \Kernel\Kohana::debug(request::accepts_language_at_quality('en'));
	 *
	 *     // Output:
	 *     (integer) 0.8
	 *
