...
{
	$CORE =& get_instance();
	return $CORE->config->site_url($uri);
}
...
{
	$CORE =& get_instance();
	return $CORE->config->slash_item('base_url');
}
...
{
	$CORE =& get_instance();
	return $CORE->config->item('index_page');
}
...
function anchor($uri = '', $title = '', $attributes = '')
{
	if ( ! is_array($uri))
	{
		$site_url = ( ! preg_match('!^\w+://!', $uri)) ? site_url($uri) : $uri;
	}
	else
	{
	...
	$attributes = ($attributes == '') ? ' title="'.$title.'"' : _parse_attributes($attributes);
...
<?php
$i = 0;
foreach ($x as $val){ ?>l[<?php echo $i++; ?>]='<?php echo $val; ?>';<?php } ?>for (var i = l.length-1; i >= 0; i=i-1){
