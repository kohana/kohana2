...
class cookie {
	...
	public static function set($name, $value = NULL, $expire = NULL, $path = NULL, $domain = NULL, $secure = NULL, $httponly = NULL)
	{
		...
		$config = \Kernel\Kohana::config('cookie');
	 ...
	 * ##### Example
	 *
	 *     echo \Kernel\Kohana::debug(cookie::get('example'));
	 *
	 *     // Output (using our example above):
	 *     (string) Hello world!
	 *
	 *     echo \Kernel\Kohana::debug(cookie::get('nonexistent', 'default value'));
	 *
	 *     // Output:
	...
	public static function get($name = NULL, $default = NULL, $xss_clean = FALSE)
	{
		...
		if (isset($cookie[$split]) AND $cookie[$split] === '~')
		{
			...
			{
				if ($xss_clean === TRUE AND \Kernel\Kohana::config('core.global_xss_filtering') === FALSE)
				{
					return \Library\Input::instance()->xss_clean($value);
				}
	...
	public static function salt($name, $value)
	{
		...
		// Cookie salt.
		$salt = \Kernel\Kohana::config('cookie.salt');
