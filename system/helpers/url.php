...
class url {
	...
	{
		$uri = ($qs === TRUE) ? \Library\Router::$complete_uri : \Library\Router::$current_uri;

		return ($suffix === TRUE) ? $uri.\Kernel\Kohana::config('core.url_suffix') : $uri;
	}
	...
	public static function base($index = FALSE, $protocol = FALSE)
	{
		if ($protocol == FALSE)
		{
			...
			$protocol = \Kernel\Kohana::config('core.site_protocol');
		}

		// Load the site domain
		$site_domain = (string) \Kernel\Kohana::config('core.site_domain', TRUE);
		...
		if ($index === TRUE AND $index = \Kernel\Kohana::config('core.index_page'))
		{
	...
	public static function site($uri = '', $protocol = FALSE)
	{
		...
		{
			// Add path suffix
			$path .= \Kernel\Kohana::config('core.url_suffix');
		}
	...
	public static function merge(array $arguments)
	{
		if ($_GET === $arguments)
		{
			$query = \Library\Router::$query_string;
		}
		...
		return \Library\Router::$current_uri.$query;
	}
	...
	public static function title($title, $separator = '-', $ascii_only = FALSE)
	{
		...
		if ($ascii_only === TRUE)
		{
			...
			$title = \Helper\text::transliterate_to_ascii($title);
	...
	public static function redirect($uri = '', $status = '302')
	{
		...
		if ($status === '300')
		{
			...
			foreach ($uri as $link)
			{
				$output .= '<li>'.\Helper\html::anchor($link).'</li>';
			}
			$output .= '</ul>';
		...
		{
			$output = '<p>'.\Helper\html::anchor($uri).'</p>';
		}

		// Run the redirect event
		\Library\Event::run('system.redirect', $uri);

		if (strpos($uri, '://') === FALSE)
		{
			// HTTP headers expect absolute URLs
			$uri = url::site($uri, \Helper\request::protocol());
		}
		...
		\Library\Event::run('system.send_headers');
