...
class Router {
	...
	public static function setup()
	{
		...
		{
			// Load routes
			Router::$routes = \Kernel\Kohana::config('routes');
		}
		...
		if (Router::$current_uri === '')
		{
			...
			if (empty(Router::$routes['_default']))
				throw new \Kernel\Kohana_Exception('Please set a default route in config/routes.php.');
		...
		// Paths to search
		$paths = \Kernel\Kohana::include_paths();
		...
		\Kernel\Event::run('system.post_routing');
	...
	public static function find_uri()
	{
		...
		if (Router::$current_uri !== '')
		{
			if ($suffix = \Kernel\Kohana::config('core.url_suffix') AND strpos(Router::$current_uri, $suffix) !== FALSE)
			{
