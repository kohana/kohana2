...
class Validation extends \ArrayObject {
	...
	protected function callback($callback)
	{
		if (is_string($callback))
		{
			...
			elseif (method_exists('\Helper\valid', $callback))
			{
				// The callback exists in valid::
				$callback = array('\Helper\valid', $callback);
			}
		}
		...
		if ( ! is_callable($callback, FALSE))
		{
			...
			throw new \Kernel\Kohana_Exception('Callback %name% used for Validation is not callable', array('%name%' => $name));
		}
	...
	public function errors($file = NULL)
	{
		...
		else
		{
			...
			foreach ($this->errors as $input => $error)
			{
				...
				if (($message = \Kernel\Kohana::message($error_locations[0])) !== $error_locations[0])
				{
					// Found a message for this field and error
				}
				elseif (($message = \Kernel\Kohana::message($error_locations[1])) !== $error_locations[1])
				{
					// Found a default message for this field
				}
				elseif (($message = \Kernel\Kohana::message($error_locations[2])) !== $error_locations[2])
				{
