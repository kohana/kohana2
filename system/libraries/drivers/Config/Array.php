...
class Config_Array extends \Driver\Config {
	...
	public function load($group, $required = FALSE)
	{
		if ($group === 'core')
		{
			...
			{
				// Invalid config file
				throw new \Library\Kohana_Config_Exception('Your Kohana application configuration file is not valid.');
			}
		...
		if ($files = \Kernel\Kohana::find_file('config', $group, $required))
		{
