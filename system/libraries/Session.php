...
class Session {
	...
	public static function instance($session_id = NULL)
	{
		...
		{
			throw new \Kernel\Kohana_Exception('A session (SID: :session:) is already open, cannot open the specified session (SID: :new_session:).', array(':session:' => session_id(), ':new_session:' => $session_id));
		}
	...
	{
		$this->input = \Library\Input::instance();
		...
		if (Session::$instance === NULL)
		{
			...
			else
			{
				...
				\Helper\cookie::set(Session::$config['name'], $_SESSION['session_id'], Session::$config['expiration']);
			}

			// Close the session on system shutdown (run before sending the headers), so that
			// the session cookie(s) can be written.
			\Kernel\Event::add('system.shutdown', array($this, 'write_close'));

			// Singleton instance
			Session::$instance = $this;
		}

		\Library\Kohana_Log::add('debug', 'Session Library initialized');
	}
	...
	public function create($vars = NULL, $session_id = NULL)
	{
		...
		if (Session::$config['driver'] !== 'native')
		{
			...
			if ( ! \Kernel\Kohana::auto_load($driver))
				throw new \Kernel\Kohana_Exception('The :driver: driver for the :library: library could not be found',
		...
		if ( ! preg_match('~^(?=.*[a-z])[a-z0-9_]++$~iD', Session::$config['name']))
			throw new \Kernel\Kohana_Exception('The session_name, :session:, is invalid. It must contain only alphanumeric characters and underscores. Also at least one letter must be present.', array(':session:' => Session::$config['name']));
		...
		(
			...
			\Kernel\Kohana::config('cookie.path'),
			\Kernel\Kohana::config('cookie.domain'),
			\Kernel\Kohana::config('cookie.secure'),
...
//		\Kernel\Event::insert_event('system.send_headers', 0, array($this, 'write_close'));
		...
		if ( ! isset($_SESSION['_kf_flash_']))
		{
			...
			$_SESSION['user_agent'] = \Helper\request::user_agent();
		...
		if ($_SESSION['total_hits'] > 1)
		{
			...
			foreach (Session::$config['validate'] as $valid)
			{
				switch ($valid)
				{
					...
					case 'user_agent':
						if ($_SESSION[$valid] !== \Helper\request::user_agent())
							return $this->create();
					break;
	...
	public function regenerate()
	{
		...
		if (isset($_COOKIE[$name]))
		{
			...
			\Helper\cookie::set($name, $_SESSION['session_id']);
		}
	}
	...
	public function write_close()
	{
		...
		if ($run === NULL)
		{
			...
			\Kernel\Event::run('system.session_write');
	...
	public function get($key = FALSE, $default = FALSE)
	{
		...
		$result = isset($_SESSION[$key]) ? $_SESSION[$key] : \Kernel\Kohana::key_string($_SESSION, $key);
