...
class View {
	...
	public function set_filename($name, $type = NULL)
	{
		if ($type == NULL)
		{
			...
			$this->kohana_filename = \Kernel\Kohana::find_file('views', $name, TRUE);
			$this->kohana_filetype = EXT;
		}
		else
		{
			// Check if the filetype is allowed by the configuration
			if ( ! in_array($type, \Kernel\Kohana::config('view.allowed_filetypes')))
				throw new \Kernel\Kohana_Exception('The requested filetype, .:type:, is not allowed in your view configuration file', array(':type:' => $type));

			// Load the filename and set the content type
			$this->kohana_filename = \Kernel\Kohana::find_file('views', $name, TRUE, $type);
			$this->kohana_filetype = \Kernel\Kohana::config('mimes.'.$type);
	...
	public function &__get($key)
	{
		...
		{
			throw new \Kernel\Kohana_Exception('Undefined view variable: :var',
				array(':var' => $key));
		}
	}
	...
	public function __toString()
	{
		...
		{
			\Kernel\Kohana_Exception::handle($e);
			return (string) '';
		}
	}
	...
	public function render($print = FALSE, $renderer = FALSE, $modifier = FALSE)
	{
		if (empty($this->kohana_filename))
			throw new \Kernel\Kohana_Exception('You must set the the view filename before calling render');
