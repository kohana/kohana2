 ...
 * @link        http://kohanaphp.com/user_guide/en/libraries/image_lib.html
 */
class Image_lib_Core {
	...
	public function initialize($props = array())
	{
		 ...
		 *
		 */
		if ($this->maintain_ratio === TRUE AND ($this->width != '' AND $this->height != ''))
		{
	...
	private function image_rotate_gd()
	{
		...
		{
			$this->set_error('rotate_unsupported');
			return FALSE;
		}
	 ...
	 * @return	resource
	 */
	private function image_create_gd($path = '', $image_type = '')
	{
		if ($path == '')
	 ...
	 * @return	mixed
	 */
	private function get_image_properties($path = '', $return = FALSE)
	{
	...
	private function set_error($msg)
	{
		if (is_array($msg))
		{
			foreach ($msg as $val)
			{
				...
				$msg = (Kohana::lang('image.'.$val) == FALSE) ? $val : Kohana::lang('image.'.$val);
				$this->error_msg[] = $msg;
				Log::add('error', $msg);
			}
		}
